...
- The deterministic `scripts/cpc_change_event_router.py` is the daytime event source for CPC package and O'Regan's inventory changes. It polls the compact production summary every five minutes from 09:00 to 19:00 Halifax time, persists latest-state work, and starts at most one exact owner per cycle. Keep scheduled owner runs as a safety net; do not add a second polling automation or wake multiple Chrome owners for one change.
