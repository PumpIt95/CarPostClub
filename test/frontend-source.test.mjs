...
test("home page gates uploads behind inventory car selection", async () => {
  ...
  assert.doesNotMatch(html, /id="notificationSettingsForm"/);
  assert.doesNotMatch(html, /id="notificationSettingsList"/);
  assert.doesNotMatch(html, /id="notificationSettingsSave"/);
  assert.doesNotMatch(html, /id="notificationSettingsToggle"/);
  assert.doesNotMatch(html, /aria-controls="notificationSettingsForm"/);
  assert.doesNotMatch(html, /Push alert types/);
...
test("frontend sends dealership, inventory filter, and vin with uploads", async () => {
  ...
  assert.doesNotMatch(source, /notificationSettingsOpen: false/);
  assert.doesNotMatch(source, /function setNotificationSettingsOpen\(/);
  assert.doesNotMatch(source, /notificationSettingsToggle/);
  assert.doesNotMatch(source, /notificationSettingsForm/);
  assert.match(source, /from "\.\/push-notification-preferences\.js"/);
  assert.doesNotMatch(source, /pushNotificationPreferenceOptions as pushNotificationSettingOptions/);
  assert.doesNotMatch(source, /isPushNotificationPreferenceKey/);
  assert.doesNotMatch(source, /function renderPushNotificationSettings\(/);
  assert.doesNotMatch(source, /function savePushNotificationSettings\(/);
  ...
  assert.match(source, /window\.location\.assign\("\/chat"\)/);
  ...
  assert.match(source, /const lockMainShell = state\.chatOpen && !dedicatedChatPage/);
  assert.match(source, /els\.appShell\.inert = lockMainShell/);
  assert.match(source, /els\.appShell\.toggleAttribute\("inert", lockMainShell\)/);
  assert.match(source, /els\.appShell\.setAttribute\("aria-hidden", String\(lockMainShell\)\)/);
...
test("mobile chat view and chat messages have distinct author accents", async () => {
  ...
  assert.match(styles, /html\.is-chat-route \.app-shell,[\s\S]*html\.is-chat-route \.app-header\s*\{[^}]*display: none;/);
  assert.match(styles, /\.app-shell\.is-chat-page\s*\{[^}]*min-height: 0;/s);
  assert.match(styles, /html\.is-chat-route \.chat-panel\s*\{[^}]*height: clamp\(520px, calc\(var\(--app-viewport-height, 100svh\) - 48px\), 900px\);/s);
  assert.match(styles, /html\.is-chat-route \.chat-panel\s*\{[^}]*margin: var\(--spacing-24\) auto;/s);
  assert.match(styles, /@media \(max-width: 680px\)[\s\S]*html\.is-chat-route \.chat-panel\s*\{[^}]*height: calc\(var\(--app-viewport-height, 100svh\) - 16px\);/);
  assert.match(styles, /@media \(max-width: 680px\)[\s\S]*html\.is-chat-route \.chat-panel\s*\{[^}]*margin: var\(--spacing-8\);/);
  assert.match(styles, /body\.chat-screen-active\s*\{[^}]*position: fixed;/s);
  assert.match(styles, /html\.is-chat-route,[\s\S]*body\.chat-screen-active\s*\{[^}]*overscroll-behavior: none;/);
  ...
  assert.match(source, /function handleViewportChange/);
  assert.match(source, /function focusChatInput/);
  assert.match(source, /els\.chatInput\.focus\(\{ preventScroll: true \}\)/);
  assert.match(source, /function handleChatInputPointerDown/);
  assert.match(source, /function queueChatViewportStabilization/);
...
test("pwa haptics provide tactile feedback on mobile interaction paths", async () => {
  ...
  assert.match(source, /els\.chatToggle\.addEventListener\("click"/);
